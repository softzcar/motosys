...
<template>
  <div class="card p-4">
    ...
    <Dialog v-model:visible="detailsModal" :style="{ width: '720px' }" modal :closable="!loadingDetails" class="p-fluid">
      <template #header>
      ...
      <div v-else-if="selectedCompra" class="space-y-6 pt-2">
        <!-- Banner ANULADA -->
        <div v-if="selectedCompra.anulada" class="bg-rose-50 border-2 border-rose-200 rounded-xl p-4">
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
              ...
              <p class="text-sm text-rose-900 font-medium mb-2">{{ selectedCompra.motivo_anulacion }}</p>
              <p class="text-xs text-rose-700">
                Anulada por <b>{{ selectedCompra.anulada_por_perfil?.nombre ?? 'Sistema' }}</b>
                el {{ formatDateTime(selectedCompra.anulada_at) }}
              </p>
              <p v-if="selectedCompra.reemplazo" class="text-xs text-rose-700 mt-2 flex items-center gap-1">
                <RotateCcw :size="12" />
                Reemplazada por compra <b>#{{ selectedCompra.reemplazo.numero }}</b> ({{ formatDate(selectedCompra.reemplazo.fecha) }})
              </p>
            </div>
          </div>
        </div>
        ...
        <!-- Banner CORRECCIÓN -->
        <div v-if="selectedCompra.corrige" class="bg-amber-50 border border-amber-200 rounded-xl p-3">
          <div class="flex items-center gap-2 text-xs text-amber-900">
            <FilePlus :size="14" />
            Esta compra reemplaza a la compra anulada <b>#{{ selectedCompra.corrige.numero }}</b> ({{ formatDate(selectedCompra.corrige.fecha) }})
          </div>
        </div>
        ...
        <div class="flex justify-between items-start bg-slate-50 p-4 rounded-xl border border-slate-200">
          <div>
            ...
            <p class="font-black text-slate-800 text-lg leading-none">{{ selectedCompra.proveedores?.nombre }}</p>
            <p class="text-xs text-slate-500 mt-1">
              <span v-if="selectedCompra.proveedores?.telefono">Tel: {{ selectedCompra.proveedores.telefono }}</span>
            </p>
          </div>
          <div class="text-right">
            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">N° FACTURA</p>
            <p class="font-black text-slate-900">{{ selectedCompra.numero_factura }}</p>
            <p class="text-xs text-slate-500 font-medium mt-1">Fecha: {{ formatDate(selectedCompra.fecha) }}</p>
          </div>
        </div>
        ...
        <div class="overflow-x-auto rounded-xl border border-slate-200">
          <table class="w-full text-left bg-white">
            ...
            <tbody class="text-sm border-b border-slate-100">
              <tr v-for="item in selectedCompra.detalle_compras" :key="item.id" class="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
        ...
        <div class="flex flex-col gap-2 bg-blue-50/50 p-4 rounded-xl border border-blue-100 mt-4">
          <div class="flex justify-between items-center text-sm" v-if="selectedCompra.subtotal">
            <span class="text-slate-500 font-bold uppercase">Subtotal</span>
            <span class="font-bold text-slate-700">{{ formatCurrency(selectedCompra.subtotal) }}</span>
          </div>
          <div class="flex justify-between items-center text-sm border-b border-blue-200/50 pb-2" v-if="selectedCompra.iva !== undefined">
            <span class="text-slate-500 font-bold uppercase">IVA</span>
            <span class="font-bold text-slate-700">{{ formatCurrency(selectedCompra.iva) }}</span>
          </div>
          <div class="flex justify-between items-center pt-1">
            <span class="text-xs font-bold text-blue-600 uppercase tracking-widest">TOTAL FACTURA</span>
            <span class="text-2xl font-black text-blue-700">{{ formatCurrency(selectedCompra.total) }}</span>
          </div>
        </div>
      </div>
