              ...
              <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div class="flex flex-col gap-2">
                    ...
                    <InputNumber v-model="empresa.iva" mode="decimal" :minFractionDigits="1" :maxFractionDigits="2" suffix="%" placeholder="Ej: 16" locale="en-US" @focus="$event => ($event.target as HTMLInputElement).select()" />
                  </div>
                </div>
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-6">
                       <div v-for="tasa in tasas" :key="tasa.codigo" class="border border-slate-200 rounded-xl p-4 bg-slate-50 flex flex-col justify-between">
                          ...
                          <div class="flex items-center gap-2">
                             <InputNumber v-model="tasa.tasa" :disabled="tasa.is_auto" mode="decimal" :minFractionDigits="2" :maxFractionDigits="4" class="w-full" :class="{'opacity-70': tasa.is_auto}" locale="en-US" @focus="$event => ($event.target as HTMLInputElement).select()" />                             <span class="text-sm font-bold text-slate-400 w-12 text-center">{{ tasa.codigo === 'COP' ? 'COP' : 'Bs' }}</span>
                          </div>
